...
ssh -i ~/.ssh/id_ed25519 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "root@${VPS_HOST}" \
  "sudo -u ${VPS_USER} bash -c 'export PATH=\$PATH:/usr/bin; cd ${APP_DIR} && npm ci && env NEXT_PUBLIC_RCS_API=https://risecorestudio.com/api NEXT_PUBLIC_RCS_COOKIE_DOMAIN=risecorestudio.com npm run build'"
