      ...
      <header className="relative">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={() => {
              const markdownContent = `# Sprint Handover Documentation: ${project.name}
**Client:** ${project.clientName || "RiseCore Client"}  
**Status:** ${project.health.toUpperCase()}  
**Deadline:** ${project.deadline || "TBD"}  
**Owner:** ${project.ownerName || "Not assigned"}  
...
## Delivery Team
${project.team.map((m) => `- **${m.name}** (${m.role.toUpperCase()} • ${m.skillLevel})`).join("\n")}

## Milestones & Sign-Offs
${project.milestones.map((m) => `- [${m.status === "complete" ? "x" : " "}] **${m.title}** (Due: ${m.dueDate})`).join("\n")}

---
*Generated by RiseCoreStudio Automated Delivery Engine — ${new Date().toISOString()}*
`;
              const blob = new Blob([markdownContent], { type: "text/markdown" });
              const url = URL.createObjectURL(blob);
              const a = document.createElement("a");
              a.href = url;
              a.download = `${project.name.replace(/\s+/g, "_")}_Handover_Doc.md`;
              a.click();
              URL.revokeObjectURL(url);
              toast("success", "Sprint Handover Documentation (.md) downloaded!");
            }}
            className="rounded-full border border-rise-accent/40 bg-rise-accent/10 px-4 py-2 text-xs font-semibold text-rise-accent hover:bg-rise-accent hover:text-rise-bg transition-colors shadow-lg"
          >
            📄 Export Handover Docs (.md)
          </button>
        </div>
